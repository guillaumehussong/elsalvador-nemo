...
async function fetchBin(path: string, onProgress: (loaded: number, total: number) => void): Promise<ArrayBuffer> {
...
function populateDeptSelect(manifest: Manifest) {
  const sel = document.getElementById("deptFilter") as HTMLSelectElement;
...
async function main() {
  ...
  const scopeBtn = document.getElementById("scopeToggle") as HTMLButtonElement;
  ...
  let sexFilter: number | null = null;
  let areaFilter: number | null = null;
  let revealProgress = reducedMotion ? 1 : 0;
  let count = manifest.row_count;
  let positions = new Float32Array(0);
  let attrs: Uint8Array = new Uint8Array(0);
  let colors: Uint8Array = new Uint8Array(0);
  let eligible: Uint32Array = new Uint32Array(0);
  ...
  function rebuildEligible() {
    ...
    for (let i = 0; i < count; i++) {
      if (passesFilter(attrs, i, deptFilter, sexFilter, areaFilter)) ids.push(i);
    }
  ...
  function makePersonaLayer(visibleLimit: number) {
  ...
  function fitScope(duration = 900) {
    const b =
      scope === "san_salvador" && manifest.san_salvador_bounds
  ...
  function showPersona(info: PickingInfo) {
    if (info.index == null || info.index < 0 || !dataReady) return;
  ...
  (document.getElementById("colorMode") as HTMLSelectElement).onchange = (e) => {
    colorMode = (e.target as HTMLSelectElement).value;
    if (dataReady) refreshColors();
  };
  (document.getElementById("deptFilter") as HTMLSelectElement).onchange = (e) => {
    deptFilter = (e.target as HTMLSelectElement).value === "" ? null : Number((e.target as HTMLSelectElement).value);
    if (dataReady) {
      rebuildEligible();
      updateLayer();
    }
  };
  (document.getElementById("sexFilter") as HTMLSelectElement).onchange = (e) => {
    sexFilter = (e.target as HTMLSelectElement).value === "" ? null : Number((e.target as HTMLSelectElement).value);
    if (dataReady) {
      rebuildEligible();
      updateLayer();
    }
  };
  (document.getElementById("areaFilter") as HTMLSelectElement).onchange = (e) => {
    areaFilter = (e.target as HTMLSelectElement).value === "" ? null : Number((e.target as HTMLSelectElement).value);
    if (dataReady) {
      rebuildEligible();
      updateLayer();
    }
  ...
  function startReveal() {
    ...
    const tick = () => {
      ...
      updateLayer();
      if (step < REVEAL_STEPS) window.setTimeout(tick, REVEAL_MS / REVEAL_STEPS);
    };
    tick();
  }

  if (map.loaded()) startReveal();
  else map.once("idle", startReveal);
}
